/**
 * Mobile AI Workout of the Week Endpoint
 *
 * GET /api/mobile/ai/workout-of-the-week
 *
...
async function getRecentWorkouts(userId: string, days: number = 7): Promise<DynamoDBWorkout[]> {
  try {
    ...
    console.error("[Mobile WOW] Error fetching recent workouts:", error);
    return [];
  }
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.log(`[Mobile WOW:${requestId}] Request from user ${userId}, force=${forceGenerate}`);
    ...
    if (!isAdmin && tier === "free") {
      return NextResponse.json(
        {
          error: "Workout of the Week is only available to paid subscribers. Upgrade to Core or Pro to unlock this feature.",
        },
        { status: 403 }
      );
    }
    ...
    if (!forceGenerate) {
      ...
      const weeklyWorkout = allScheduled.find(
        (w) =>
      ...
      if (weeklyWorkout) {
        console.log(`[Mobile WOW:${requestId}] Returning scheduled workout for this week`);
        return NextResponse.json({
          workout: weeklyWorkout,
          isNew: false,
          rationale: "You have a workout of the week already scheduled. Complete this to stay on track!",
        });
      }
    }
    ...
    console.log(`[Mobile WOW:${requestId}] Generating workout...`);
    console.log(`[Mobile WOW:${requestId}] Focus: ${workoutFocus}`);
    ...
    if (!validation.valid) {
      console.error(`[Mobile WOW:${requestId}] Validation failed:`, validation.errors);
      return NextResponse.json(
    ...
    console.log(`[Mobile WOW:${requestId}] Success: ${savedWorkout.title}`);
    console.log(`[Mobile WOW:${requestId}] Exercises: ${exercises.length}`);
    ...
    console.error(`[Mobile WOW:${requestId}] Error:`, error);
    return NextResponse.json(
